...
def main():
    ...
    # Load HAMER
    logger.stage("Loading HAMER")
    from hamer.models import load_hamer
    m_hamer, cfg_h = load_hamer(CKPT, init_renderer=False)
    dev = "cuda" if torch.cuda.is_available() else "cpu"
    m_hamer = m_hamer.to(dev).eval()
    logger.ok(f"HAMER loaded ({sum(p.numel() for p in m_hamer.parameters())/1e6:.0f}M params)")

    # Load ViTPose
    logger.stage("Loading ViTPose")
    try:
        from vitpose_model import ViTPoseModel
        vitpose = ViTPoseModel(dev)
        logger.ok("ViTPose ready")
    except Exception as e:
        logger.fail(f"ViTPose: {e}")
        print("[ViTPose] Visual angle observation disabled. Fusion will use IMU only.")
        vitpose = None

    # Open D435i
    logger.stage("Opening D435i")
    ...
    imu_thread.start()
    logger.info("IMU reader thread started on " + COM_PORT)
        ...
        while True:
            ...
            if vit_skip <= 0:
                ...
                try:
                    ...
                    print(f"[ViTPose] Predict error: {e}")
                    vitposes = []
            ...
            if frame_count % 30 == 0:
                ...
                if ran_vitpose:
                    _perf_vit = f"{_t_vit:.0f}ms"
                else:
                    _perf_vit = "skip"
                _perf_hamer = f"{_t_hamer:.0f}ms" if visual.valid else "idle"
                print(f"[PERF] CPU={_cpu_mode} total={_t_total:.0f}ms vit={_perf_vit} hamer={_perf_hamer} fps={current_fps:.1f}", flush=True)
                logger.perf(f"total={_t_total:.0f}ms vit={_perf_vit} hamer={_perf_hamer} fps={current_fps:.1f}")
...
def run_diagnostic():
    """Quick startup check - runs before main()"""
    issues = []
    for port, name in [(5055, "VisionBridge"), (8080, "FusionUDP")]:
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        try:
            s.bind(("127.0.0.1", port))
            s.close()
        except OSError:
            issues.append(f"Port {port} ({name}) already in use")

    import serial
    try:
        ser = serial.Serial(COM_PORT, COM_BAUD, timeout=0.1)
        ser.close()
    except Exception as e:
        issues.append(f"IMU {COM_PORT}: {str(e).split(chr(40))[0] if chr(40) in str(e) else str(e)}")

    try:
        import pyrealsense2 as rs
        if len(list(rs.context().devices)) == 0:
            issues.append("D435i: No camera found")
    except:
        issues.append("D435i: pyrealsense2 import failed")

    if issues:
        print("=" * 50)
        print("  STARTUP WARNINGS (" + str(len(issues)) + " issues):")
        for i in issues:
            print("    [?] " + i)
        print("=" * 50)
        print()
        return False
    print("[DIAG] All startup checks passed\n")
...
def run_diagnostic():
    ...
    try:
        import serial
        ser = serial.Serial(COM_PORT, COM_BAUD, timeout=0.1); ser.close()
    except Exception as e:
        issues.append(f"IMU {COM_PORT}: {str(e).split(chr(40))[0]}")
    try:
        import pyrealsense2 as rs
        if len(list(rs.context().devices)) == 0:
            issues.append("D435i: No camera found")
...
def _run_with_restart():
    while True:
        try:
            main()
            print("[RESTART] Pipeline exited normally. Not restarting.")
            break
        except KeyboardInterrupt:
            print("\n[RESTART] Stopped by user. Exiting.")
            break
        except Exception as _e:
            print(f"[RESTART] Pipeline crashed: {_e}")
            print(f"[RESTART] Restarting in {_RESTART_DELAY}s (Ctrl+C to stop)...")
            import time as _t
            _t.sleep(_RESTART_DELAY)
            print(f"[RESTART] Restarting...")
